...
// const fetchPrompt = async (): Promise<{
//   prompt: string;
//   prompt_id: number;
// }> => {
...
export function RichTextEditorDemo({
  ...
  const [writerBlockPrompt, setWriterBlockPrompt] = useState<string | null>(
    null
  );
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "max-w-full focus:outline-none prose prose-sm sm:prose lg:prose-lg xl:prose-2xl mx-auto",
      },
    },
    onUpdate: ({ editor }) => {
  ...
  //         : `${FLASK_API_URL}/v1/entries/${entryId}`;
  ...
  //       if (!response.ok) {
  //         throw new Error(
  //           `Failed to ${isNewEntry ? "create" : "update"} entry`
  //         );
  //       }
  ...
  //TODO: Implementing autosave using react autosave
  useAutosave({
    data: editor ? editor.getHTML() : "",
  ...
  const loadPrompt = useCallback(async () => {
    if (isNewJournal && editor && isMounted) {
      try {
        const token = await getToken();
        const response = await fetch(`${FLASK_API_URL}/v1/prompts`, {
          headers: {
        ...
        const { prompt, prompt_id } = await response.json();
        setPromptId(prompt_id);
        console.log(`Prompt used is: ${prompt}`);
  ...
  useEffect(() => {
    if (entryId != null && !isNewJournal && editor && isMounted) {
      console.log("entryId: ", entryId);
      (async () => {
        try {
          const token = await getToken();
          const res = await fetch(
            `${FLASK_API_URL}/v1/entries/entryId/${entryId}`,
            {
              credentials: "include",
              headers: {
                Authorization: `Bearer ${token}`,
  ...
  if (!isMounted || !editor) {
    return (
      <div
        className={cn("relative w-full border bg-card rounded-lg", className)}
      >
  ...
  return (
    <div className="">
      <div className="w-full max-w-screen-md lg:max-w-screen-lg mx-auto h-[calc(100dvh-10.75rem)] sm:h-[calc(100dvh-12rem)] relative overflow-hidden border bg-card shadow-lg rounded-xl">
        <EditorToolbar editor={editor} />
        <div
          ref={editorRef}
          className="overflow-y-auto overflow-x-auto h-[calc(100%-50px)] relative overscroll-contain touch-pan-y"
