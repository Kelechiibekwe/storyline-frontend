...
const replaceAll = (replaceTerm: string, results: Range[], { tr, dispatch }: { tr: any; dispatch: any }) => {
  if (!results.length) {
    return
  }
...
const selectNext = (editor: CoreEditor) => {
  const { results } = editor.storage.searchAndReplace as SearchAndReplaceStorage
